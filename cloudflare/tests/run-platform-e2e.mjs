...
const eventPromise=new Promise((resolve,reject)=>{const t=setTimeout(()=>reject(new Error("ws broadcast timeout")),5000);ws.addEventListener("message",e=>{clearTimeout(t);resolve(JSON.parse(e.data))},{once:true})});
