...
PWAIT="$(curl -sS -X POST "$BASE/api/passenger/waiting" -H 'Content-Type: application/json' -H 'X-Passenger-Id: e2e-passenger' -d '{"routeId":"e2e-route","originPointId":"e2e-origin","destinationPointId":"e2e-destination","pickupPointId":"e2e-origin","requestMode":"RANK","groupSize":2}')"
printf '%s' "$PWAIT" | grep -q '"ok":true' && ok "Passenger RANK waiting" || bad "Passenger RANK waiting" "$PWAIT"
WAITING_ID="$(printf '%s' "$PWAIT" | value waiting.id)"
PDUP="$(curl -sS -X POST "$BASE/api/passenger/waiting" -H 'Content-Type: application/json' -H 'X-Passenger-Id: e2e-passenger' -d '{"routeId":"e2e-route","originPointId":"e2e-origin","destinationPointId":"e2e-destination","pickupPointId":"e2e-origin","requestMode":"RANK","groupSize":2}')"
DUP_ID="$(printf '%s' "$PDUP" | value waiting.id)"
[[ -n "$WAITING_ID" && "$WAITING_ID" == "$DUP_ID" ]] && ok "Passenger duplicate waiting idempotency" || bad "Passenger duplicate waiting idempotency" "$PDUP"
...
PAX="$(curl -sS -X POST "$BASE/api/driver/passengers" -H "Authorization: Bearer $DR_TOKEN" -H 'Content-Type: application/json' -d '{"tripId":"'"$TRIP_ID"'","passengersOnboard":15}')"
...
printf '%s' "$FK" | grep -q '"results": \[\]' && ok "D1 foreign key integrity query" || bad "D1 foreign key integrity query" "$FK"
